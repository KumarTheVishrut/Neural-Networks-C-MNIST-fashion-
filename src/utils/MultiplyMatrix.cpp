#include "../../include/utils/MultiplyMatrix.cpp"

utils::MultiplyMatrix::Multiply(Matrix *a , Matrix *b){

	this->a = a;
	this->b = b;

	if (a->getNumCols() != getNumRows()){
	cerr<< "A_rows" << a->getNumRows() << "!= B_cols" << b->getNumCols()<< endl;
	assert(false);


	}
	this->c = new Matrix(a->getNumRows(),b->getNumCols(),false)

}



Matrix *utils::MultiplyMatrix::execute(){

	for(int i =0 ; i < a->getNumRows(); i++){
	   for(int j = 0 ; j < b->getNumCols();j++){
		for(int k = 0; k < b->getNumRows();k++){
		double p = this->a->getValue(i,k) * this->b->getValue(k,l);
		double newVal = this->c->getValue(i,j) + p;
		this->c->setValue(i,j,newVal);
		}

	   } 

	}

	return this->c;

}
